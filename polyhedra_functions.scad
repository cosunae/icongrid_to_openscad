// extracted from https://github.com/KitWallace/openscad
...
function lower(char) =
    len(search(char,"abcdefghijklmnopqrstuvwxyz")) == 1 ;
...
//      import(file=str("write/",font,".dxf"),layer=char_layer(char));
      import(file=str("Letters.dxf"),layer=char_layer(char));
};

module write_line(line,font,width){
   for (i = [0:len(line)-1])
      translate([i*width,0,0])
        write_char(font,line[i]);
};

module write_lines(lines,font,width,linespace) {
   for (j=[0:len(lines)-1])
     translate([0,j*linespace,0])
        write_line(lines[j],font,width);
};
